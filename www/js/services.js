angular.module("gsja_tesalonika.services", [])
// TODO: --|---- directive
...
.directive("barcodeScanner", function($compile, $ionicModal, $ionicPlatform) {
	return {
		scope: {
			barcodeText: "="
		},
		link: function link($scope, $element, $attrs) {
			$scope.barcodeText = "";
			$element.bind("click", runScanner);
			function runScanner(event) {
				$scope.barcodeText = "";
				if (window.cordova && window.cordova.plugins.barcodeScanner) {
					cordova.plugins.barcodeScanner.scan(function(result) {
						$scope.$apply(function() {
							$scope.barcodeText = result.text;
						});
					},function(error) {
						$scope.form_contact.Barcode = "Scanning failed: " + error;
					},{
					...
					$scope.$apply(function() {
						$scope.barcodeText = "Only work in real device!";
					});
				}
			}
		}
